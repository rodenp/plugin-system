  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex-1 bg-white">
        {(() => {
          const ActivePluginComponent = installedPlugins.find(p => p.id === activeTab)?.component;
          ...
          const baseProps = {
            ...
            onMarkThreadAsResolved: handleMarkThreadAsResolved,
            ...
            ...(useStorageManager && storage.isInitialized
              ? {
                  onSaveData: async (entityType: string, data: any[]) => {
                    try {
                      await storage.clear(entityType);
                      if (data.length > 0) {
                        await storage.createMany(entityType, data);
                      }
                      console.log(`✅ Saved ${data.length} ${entityType} items via storage manager`);
                    } catch (error) {
                      console.error(`❌ Failed to save ${entityType} via storage manager:`, error);
                      throw error;
                    }
                  },
                  onLoadData: async (entityType: string) => {
                    try {
                      const result = await storage.query(entityType);
                      console.log(`✅ Loaded ${result.length} ${entityType} items via storage manager`);
                      return result;
                    } catch (error) {
                      console.error(`❌ Failed to load ${entityType} via storage manager:`, error);
                      throw error;
                    }
                  }
                }
