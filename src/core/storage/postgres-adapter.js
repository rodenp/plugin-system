...
export class PostgresAdapter extends BaseStorageAdapter {
    config;
    db; // Would be your DB client (pg, Prisma, etc.)
    constructor(config) {
        super();
        this.config = {
    ...
    async saveCourse(course) {
      ...
      `;
            ...
            await this.db.query(`DELETE FROM ${this.config.tablePrefix}modules WHERE course_id = $1`, [course.id]);
            // Insert modules
    ...
    async deleteCourse(id) {
        ...
        try {
            ...
            await this.db.query(`DELETE FROM ${this.config.tablePrefix}modules WHERE course_id = $1`, [id]);
            // Delete course
            await this.db.query(`DELETE FROM ${this.config.tablePrefix}courses WHERE id = $1`, [id]);
            await this.db.query('COMMIT');
        }
        catch (error) {
            await this.db.query('ROLLBACK');
