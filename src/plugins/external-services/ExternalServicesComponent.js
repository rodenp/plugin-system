...
// COMPONENT
// ============================================================================
export const ExternalServicesComponent = ({ currentUser, communityId, community, userRole, theme, services = {}, statuses = {}, usage = {}, webhookEvents = [], loading = false, error, onInitializeServices, onTestConnection, onSendEmail, onUploadFile, onGenerateContent, onAddService, onUpdateService, onRemoveService, onSendWebhook, }) => {
    // Apply theme
    ...
    const totalRequests = Object.values(usage).reduce((sum, serviceUsage) => sum + serviceUsage.reduce((total, record) => total + record.requestCount, 0), 0);
    const totalErrors = Object.values(usage).reduce((sum, serviceUsage) => sum + serviceUsage.reduce((total, record) => total + record.errorCount, 0), 0);
    // Helper functions
    ...
    const handleSendTestEmail = async () => {
        if (onSendEmail) {
            try {
                await onSendEmail(currentUser.email, 'Test Email from External Services', '<h1>Test Email</h1><p>This is a test email sent from the External Services plugin.</p>', undefined, { userName: currentUser.profile.displayName });
            }
            catch (error) {
    ...
    const handleGenerateTestContent = async () => {
        if (onGenerateContent) {
            try {
                await onGenerateContent('Write a short welcome message for a new course platform user', 'text', { maxTokens: 100, temperature: 0.7 });
            }
            catch (error) {
    ...
    }, ['services', 'status', 'usage', 'webhooks', 'test'].map(tab => React.createElement('button', {
        key: tab,
    ...
    const renderServices = () => (React.createElement('div', {}, renderOverview(), React.createElement('div', {
        style: {
            display: 'flex',
    ...
    }, 'External Services'), (userRole === 'owner' || userRole === 'admin') && React.createElement('button', {
    ...
    }, 'Add Service')), React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.lg
        }
        ...
        Object.values(services).map(service => {
            ...
            }, getServiceTypeIcon(service.type)), React.createElement('div', {}, React.createElement('h3', {
                style: {
                    fontSize: appliedTheme.font.sizeLg,
                    fontWeight: 600,
                    margin: 0,
            ...
            }, service.name), React.createElement('p', {
                style: {
                    fontSize: appliedTheme.font.sizeSm,
                    color: appliedTheme.colors.textSecondary,
                    margin: 0
                }
            }, `${service.provider} • ${service.type}`))), React.createElement('div', {
                style: { display: 'flex', alignItems: 'center', gap: appliedTheme.spacing.md }
            }, React.createElement('div', {
            ...
            }, status.status)), React.createElement('div', {
                style: {
                    fontSize: appliedTheme.font.sizeXs,
                    color: appliedTheme.colors.textSecondary
                }
            }, `${totalRequests} requests`)), React.createElement('button', {
    ...
    const renderStatus = () => (React.createElement('div', {}, React.createElement('h2', {
        style: {
            fontSize: appliedTheme.font.sizeXl,
            fontWeight: 600,
            marginBottom: appliedTheme.spacing.lg,
    ...
    }, 'Service Status'), React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.md
        }
    }, Object.values(statuses).length > 0 ?
        Object.values(statuses).map(status => React.createElement('div', {
        ...
        }, services[status.serviceId]?.name || status.serviceId), React.createElement('p', {
            style: {
                fontSize: appliedTheme.font.sizeSm,
                color: appliedTheme.colors.textSecondary,
                margin: 0
            }
        }, `Last checked: ${status.lastChecked.toLocaleTimeString()}`)), React.createElement('div', {
            style: { textAlign: 'right' }
        }, React.createElement('div', {
            style: {
                display: 'flex',
        ...
        }, status.status)), status.responseTime && React.createElement('div', {
            style: {
                fontSize: appliedTheme.font.sizeXs,
                color: appliedTheme.colors.textSecondary
            }
    ...
    const renderWebhooks = () => (React.createElement('div', {}, React.createElement('h2', {
        style: {
            fontSize: appliedTheme.font.sizeXl,
            fontWeight: 600,
            marginBottom: appliedTheme.spacing.lg,
    ...
    }, 'Webhook Events'), React.createElement('div', {
        style: {
            backgroundColor: appliedTheme.colors.surface,
            borderRadius: appliedTheme.borders.borderRadius,
            padding: appliedTheme.spacing.lg,
        ...
        }, webhookEvents.slice(0, 10).map(event => React.createElement('div', {
            key: event.id,
            style: {
        ...
        }, event.event), React.createElement('span', {
            style: {
                padding: `${appliedTheme.spacing.xs} ${appliedTheme.spacing.sm}`,
                borderRadius: appliedTheme.borders.borderRadius,
                fontSize: appliedTheme.font.sizeXs,
        ...
        }, event.status)), React.createElement('p', {
            style: {
                fontSize: appliedTheme.font.sizeXs,
                color: appliedTheme.colors.textSecondary,
                margin: 0
    ...
    const renderTestInterface = () => (React.createElement('div', {}, React.createElement('h2', {
        style: {
            fontSize: appliedTheme.font.sizeXl,
            fontWeight: 600,
            marginBottom: appliedTheme.spacing.lg,
    ...
    }, 'Test Services'), React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.lg
        }
    ...
    }, 'Email Service Test'), React.createElement('p', {
        style: {
            fontSize: appliedTheme.font.sizeSm,
            color: appliedTheme.colors.textSecondary,
            marginBottom: appliedTheme.spacing.md
        }
    }, 'Send a test email to verify email service configuration.'), React.createElement('button', {
    ...
    }, 'Send Test Email')), React.createElement('div', {
        style: {
            backgroundColor: appliedTheme.colors.surface,
            borderRadius: appliedTheme.borders.borderRadius,
            padding: appliedTheme.spacing.lg,
    ...
    }, 'AI Content Generation Test'), React.createElement('p', {
        style: {
            fontSize: appliedTheme.font.sizeSm,
            color: appliedTheme.colors.textSecondary,
            marginBottom: appliedTheme.spacing.md
        }
    }, 'Test AI content generation capabilities.'), React.createElement('button', {
    ...
    // Tab content
    !loading && activeTab === 'services' && renderServices(), !loading && activeTab === 'status' && renderStatus(), !loading && activeTab === 'webhooks' && renderWebhooks(), !loading && activeTab === 'test' && renderTestInterface(), activeTab === 'usage' && React.createElement('div', {
        style: {
            backgroundColor: appliedTheme.colors.surface,
            borderRadius: appliedTheme.borders.borderRadius,
            padding: appliedTheme.spacing.xl,
    ...
    }, 'Service Usage Analytics'), React.createElement('p', {
        style: {
            color: appliedTheme.colors.textSecondary
        }
