...
interface FeatureFlagsProps extends PluginProps {
  // Data from host app
  ...
  // Action callbacks
  onInitializeFlags?: (config: FeatureFlagsConfig) => Promise<void>;
  onLoadFlags?: (userId?: string, userContext?: Record<string, any>) => Promise<void>;
  onEvaluateFlag?: (flagId: string, userId?: string, userContext?: Record<string, any>, defaultValue?: boolean) => Promise<void>;
  onUpdateFlag?: (flag: FeatureFlag) => Promise<void>;
  onAddPermission?: (permission: PermissionRule) => void;
  onRemovePermission?: (permissionId: string) => void;
  onTestFlag?: (flagId: string, userId: string) => Promise<boolean>;
}
...
export const FeatureFlagsComponent: React.FC<FeatureFlagsProps> = ({
  currentUser,
  communityId,
  community,
  userRole,
  ...
  // Local state
  const [activeTab, setActiveTab] = React.useState<'flags' | 'permissions' | 'config' | 'test'>('flags');
  ...
  const getRolloutColor = (percentage?: number) => {
    if (!percentage) return appliedTheme.colors.muted;
    if (percentage === 100) return appliedTheme.colors.secondary;
    if (percentage >= 50) return appliedTheme.colors.warning;
  ...
  const renderTabNavigation = () => (
    React.createElement('div', {
      ...
      ['flags', 'permissions', 'config', 'test'].map(tab =>
          ...
          Object.values(flags).map(flag => {
            ...
            return React.createElement('div', {
              ...
              React.createElement('div', {
                ...
                flag.description && React.createElement('p', {
                  style: {
                    fontSize: appliedTheme.font.sizeSm,
                    color: appliedTheme.colors.textSecondary,
                    margin: 0,
                ...
                React.createElement('div', {
                  ...
                  userValue !== undefined && React.createElement('span', {},
                    `User: ${userValue ? 'Enabled' : 'Disabled'}`
                  ),
                  testResult !== undefined && React.createElement('span', {},
                    `Test: ${testResult ? 'Enabled' : 'Disabled'}`
                  )
                )
              ),
              React.createElement('div', {
            ...
            Object.values(permissions).map(permission =>
              React.createElement('div', {
                ...
                React.createElement('div', {
                  ...
                  }, userPermissions[permission.id] ? 'Granted' : 'Denied')
                ),
                React.createElement('div', {
                  style: {
                    fontSize: appliedTheme.font.sizeXs,
  ...
  const renderConfig = () => (
    React.createElement('div', {},
      ...
      featureFlagsConfig ? React.createElement('div', {
        style: {
          backgroundColor: appliedTheme.colors.surface,
          borderRadius: appliedTheme.borders.borderRadius,
          padding: appliedTheme.spacing.lg,
        ...
        React.createElement('div', {
          ...
          }, getProviderIcon(featureFlagsConfig.provider)),
          React.createElement('div', {},
            React.createElement('h3', {
              style: {
                fontSize: appliedTheme.font.sizeLg,
            ...
            }, featureFlagsConfig.environment || 'No environment specified')
          )
        ),
        React.createElement('div', {
          style: {
  ...
  const renderTest = () => (
    React.createElement('div', {},
      ...
      React.createElement('div', {
        ...
        }, 'Test feature flags for the current user and context.'),
            ...
            Object.keys(flags).map(flagId =>
              React.createElement('div', {
                ...
                React.createElement('div', {
                  ...
                  }, testResults[flagId] ? 'Enabled' : 'Disabled'),
                  React.createElement('button', {
  ...
  return React.createElement('div', {
    ...
    // Tab content
    !loading && activeTab === 'flags' && renderFlags(),
    !loading && activeTab === 'permissions' && renderPermissions(),
    !loading && activeTab === 'config' && renderConfig(),
    !loading && activeTab === 'test' && renderTest()
  );
};