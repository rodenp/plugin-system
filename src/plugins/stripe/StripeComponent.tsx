...
// ============================================================================
// TYPES (imported from original plugin)
// ============================================================================
...
interface StripeProps extends PluginProps {
  // Data from host app
  config?: StripeConfig;
  ...
  // Action callbacks
  onConfigureStripe?: (config: StripeConfig) => Promise<void>;
  onLoadCustomerData?: (customerId: string) => Promise<void>;
  onCreateCheckoutSession?: (planId: string, customerId?: string) => Promise<{ sessionId: string; url: string }>;
  onCancelSubscription?: (subscriptionId: string, immediate?: boolean) => Promise<void>;
  onResumeSubscription?: (subscriptionId: string) => Promise<void>;
  onCreateBillingPortalSession?: (customerId: string, returnUrl: string) => Promise<{ url: string }>;
  onCreateCustomer?: (params: { email: string; name?: string; metadata?: Record<string, string> }) => Promise<Customer>;
  onUpdateSubscription?: (params: { subscriptionId: string; priceId?: string; quantity?: number; cancelAtPeriodEnd?: boolean }) => Promise<Subscription>;
}
...
export const StripeComponent: React.FC<StripeProps> = ({
  currentUser,
  communityId,
  community,
  userRole,
  ...
  // Local state
  const [activeTab, setActiveTab] = React.useState<'overview' | 'plans' | 'customer' | 'billing' | 'config'>('overview');
  ...
  const renderTabNavigation = () => (
    React.createElement('div', {
      ...
      ['overview', 'plans', 'customer', 'billing', 'config'].map(tab => 
  ...
  const renderOverview = () => (
    React.createElement('div', {},
      ...
      React.createElement('div', {
        ...
        customer?.stripeCustomerId && React.createElement('button', {
      ...
      React.createElement('form', {
        ...
        React.createElement('div', {
          ...
          React.createElement('input', {
            ...
            onChange: (e: any) => setConfigForm(prev => ({ ...prev, publishableKey: e.target.value })),
        ...
        React.createElement('div', {
          ...
          React.createElement('input', {
            ...
            onChange: (e: any) => setConfigForm(prev => ({ ...prev, testMode: e.target.checked })),
            style: {
  ...
  return React.createElement('div', {
    ...
    !loading && activeTab === 'plans' && renderPlans(),
    !loading && activeTab === 'customer' && renderCustomer(),
    
    activeTab === 'billing' && React.createElement('div', {
      style: {
        textAlign: 'center',
        padding: appliedTheme.spacing.xl,
        color: appliedTheme.colors.textSecondary
