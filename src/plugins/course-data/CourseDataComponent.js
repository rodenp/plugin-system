...
// COMPONENT
// ============================================================================
export const CourseDataComponent = ({ currentUser, communityId, community, userRole, theme, courses = {}, lessons = {}, enrollments = {}, studentProgress = {}, currentCourse, currentLesson, loading = false, error, filters = {}, sortBy = 'created', sortOrder = 'desc', onLoadCourses, onLoadCourse, onCreateCourse, onUpdateCourse, onDeleteCourse, onEnrollInCourse, onUpdateProgress, onCreateLesson, onUpdateLesson, onSetFilters, onSetSorting, }) => {
    // Apply theme
    ...
    }, ['catalog', 'my-courses', 'create', 'analytics'].map(tab => React.createElement('button', {
        key: tab,
    ...
    const renderCourseCard = (course) => (React.createElement('div', {
        key: course.id,
        style: {
    ...
    }, React.createElement('span', {}, `${course.lessons.length} lessons`), course.duration && React.createElement('span', {}, formatDuration(course.duration)), React.createElement('span', {}, `${course.analytics.totalEnrollments} students`)), 
    // Footer
    React.createElement('div', {
        style: {
            display: 'flex',
    ...
    }, course.level), React.createElement('div', {
        style: {
            fontSize: appliedTheme.font.sizeLg,
            fontWeight: 600,
            color: appliedTheme.colors.textPrimary,
    ...
    }, course.price === 0 ? 'Free' : `${course.currency} ${course.price}`)), React.createElement('button', {
        onClick: (e) => {
            e.stopPropagation();
    ...
    const renderCatalog = () => (React.createElement('div', {}, 
    // Search and filters
    React.createElement('div', {
        style: {
    ...
    const renderMyCourses = () => (React.createElement('div', {}, React.createElement('h2', {
        style: {
    ...
    }, 'My Enrolled Courses'), React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.lg
        }
        ...
        userEnrollments.map(enrollment => {
            ...
            }, 'Continue'), enrollment.status === 'completed' && course.settings.certificateEnabled &&
                React.createElement('button', {
                    style: {
                        padding: `${appliedTheme.spacing.sm} ${appliedTheme.spacing.md}`,
    ...
    const renderAnalytics = () => {
        ...
        }, 'Course Analytics'), React.createElement('div', {
            style: {
                display: 'grid',
    ...
    // Tab content
    !loading && activeTab === 'catalog' && renderCatalog(), !loading && activeTab === 'my-courses' && renderMyCourses(), !loading && activeTab === 'analytics' && renderAnalytics(), activeTab === 'create' && React.createElement('div', {
        style: {
            backgroundColor: appliedTheme.colors.surface,
            borderRadius: appliedTheme.borders.borderRadius,
            padding: appliedTheme.spacing.xl,
    ...
    }, 'Create New Course'), React.createElement('p', {
        style: {
            color: appliedTheme.colors.textSecondary,
            marginBottom: appliedTheme.spacing.lg
        }
    }, 'Course creation interface would be implemented here.'), (userRole === 'owner' || userRole === 'admin') && React.createElement('button', {
