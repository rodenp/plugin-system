...
// ============================================================================
// TYPES (from original Redux plugin)
// ============================================================================
...
interface CoursePublishingProps extends PluginProps {
  // Data from host app
  ...
  // Action callbacks
  onInitializeCoursePublishing?: (courseId: string) => Promise<void>;
  onSubmitForReview?: (courseId: string, publishingInfo: Partial<CoursePublishingInfo>) => Promise<void>;
  onPublishCourse?: (courseId: string, publishingInfo: CoursePublishingInfo) => Promise<void>;
  onUnpublishCourse?: (courseId: string) => Promise<void>;
  onReviewCourse?: (courseId: string, review: Omit<CourseReview, 'id' | 'reviewedAt'>) => Promise<void>;
  onUpdateMarketplaceEntry?: (courseId: string, updates: Partial<CourseMarketplaceEntry>) => Promise<void>;
  onUpdatePublishingInfo?: (courseId: string, updates: Partial<CoursePublishingInfo>) => Promise<void>;
  onValidateCourse?: (courseId: string) => Promise<string[]>;
}
...
export const CoursePublishingComponent: React.FC<CoursePublishingProps> = ({
  currentUser,
  communityId,
  community,
  userRole,
  ...
  loading = false,
  error,
  onInitializeCoursePublishing,
  onSubmitForReview,
  onPublishCourse,
  onUnpublishCourse,
  onReviewCourse,
  onUpdateMarketplaceEntry,
  onUpdatePublishingInfo,
  onValidateCourse,
}) => {
  // Apply theme
  ...
  // Local state
  const [activeTab, setActiveTab] = React.useState<'overview' | 'publishing' | 'marketplace' | 'reviews'>('overview');
  const [activeCourse, setActiveCourse] = React.useState<string>(selectedCourse || availableCourses[0]?.id || '');
  ...
  const canSubmitForReview = (status?: CourseStatus) => {
    return status === 'draft' || status === 'rejected';
  };

  const canPublish = (status?: CourseStatus) => {
    return status === 'review' && (userRole === 'admin' || userRole === 'owner');
  };

  const canUnpublish = (status?: CourseStatus) => {
  ...
  const handleUpdateInfo = async (updates: Partial<CoursePublishingInfo>) => {
  ...
  const renderTabNavigation = () => (
    React.createElement('div', {
      ...
      ['overview', 'publishing', 'marketplace', 'reviews'].map(tab =>
  ...
  const renderCourseSelector = () => (
    React.createElement('div', {
      ...
      React.createElement('select', {
        value: activeCourse,
        onChange: (e: any) => setActiveCourse(e.target.value),
        style: {
          width: '100%',
        ...
        availableCourses.map(course =>
          React.createElement('option', { key: course.id, value: course.id }, course.title)
        )
      )
    )
  );
  ...
  const renderOverview = () => (
    React.createElement('div', {},
      ...
      React.createElement('div', {
        ...
        Object.keys(publishingInfo).length > 0 ? React.createElement('div', {
          style: {
            display: 'grid',
            ...
            return React.createElement('div', {
              ...
              React.createElement('div', {
                ...
                React.createElement('div', {
                  ...
                  }, course?.title || info.metadata.title)
                ),
                React.createElement('div', {
                  style: {
                    display: 'flex',
                  ...
                  React.createElement('span', {}, `${info.pricing} • ${info.visibility}`),
                  info.publishedAt && React.createElement('span', {}, 
  ...
  const renderPublishing = () => (
    React.createElement('div', {},
      ...
      !activeCourse ? React.createElement('p', {
        style: {
          textAlign: 'center',
          color: appliedTheme.colors.textSecondary,
        ...
        React.createElement('div', {
          ...
          React.createElement('div', {
            ...
            React.createElement('div', {},
              ...
              }, currentCourse?.title || courseInfo?.metadata.title),
              courseInfo && React.createElement('div', {
                ...
                React.createElement('span', {
                  style: { fontSize: appliedTheme.font.sizeLg }
                }, getStatusIcon(courseInfo.status)),
                React.createElement('span', {
                  style: {
                    padding: `${appliedTheme.spacing.xs} ${appliedTheme.spacing.sm}`,
                    borderRadius: appliedTheme.borders.borderRadius,
            ...
            React.createElement('div', {
              ...
              courseInfo && canSubmitForReview(courseInfo.status) && React.createElement('button', {
              ...
              courseInfo && canPublish(courseInfo.status) && React.createElement('button', {
              ...
              courseInfo && canUnpublish(courseInfo.status) && React.createElement('button', {
  ...
  const renderReviews = () => (
    React.createElement('div', {},
      ...
      React.createElement('div', {
        ...
        courseReviews.length > 0 ? React.createElement('div', {
          style: {
            display: 'grid',
            gap: appliedTheme.spacing.md
          }
          ...
          courseReviews.map(review =>
            React.createElement('div', {
              ...
              React.createElement('div', {
                ...
                Object.entries(review.checklist).map(([key, passed]) =>
                  React.createElement('div', {
                    key: key,
                    style: {
                      display: 'flex',
                    ...
                    React.createElement('span', {}, passed ? '✅' : '❌'),
                    React.createElement('span', {
  ...
  return React.createElement('div', {
    ...
    !loading && activeTab === 'publishing' && renderPublishing(),
    !loading && activeTab === 'marketplace' && renderMarketplace(),
