...
export class CommunityService {
    ...
    // Posts and content
    async getCommunityPosts(communityId, category, limit = 20, offset = 0) {
    ...
    async checkMemberAccess(communityId, userId) {
        try {
    ...
    async inviteMember(communityId, email) {
...
export function useCommunity(communityId) {
    ...
    React.useEffect(() => {
        if (communityId) {
            ...
            const service = window.__pluginManager?.getState('communityService');
            if (service) {
...
export function useCommunityPosts(communityId, category) {
...
// Plugin factory
export const createCommunityPlugin = (config) => {
    ...
    return {
        ...
        hooks: {
            ...
            useCommunityManagement: () => {
                return {
                    createCommunity: (data) => communityService.createCommunity(data),
                    updateCommunity: (id, updates) => communityService.updateCommunity(id, updates),
                    deleteCommunity: (id) => communityService.deleteCommunity(id),
                    addMember: (communityId, userId, role) => communityService.addMemberToCommunity(communityId, userId, role),
                    removeMember: (communityId, userId) => communityService.removeMemberFromCommunity(communityId, userId),
                    inviteMember: (communityId, email) => communityService.inviteMember(communityId, email),
                    checkAccess: (communityId, userId) => communityService.checkMemberAccess(communityId, userId)
                };
            }
        },
