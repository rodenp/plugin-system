...
// COMPONENT
// ============================================================================
export const AuthComponent = ({ currentUser, communityId, community, userRole, theme, currentSession, authConfig, users = [], loginAttempts = {}, lockedUsers = {}, loading = false, error, onSignIn, onSignUp, onSignOut, onUpdateUser, onDeleteUser, onResetPassword, onUpdatePassword, onSendVerificationEmail, onVerifyEmail, onSignInWithProvider, onUnlockUser, onUpdateAuthConfig, }) => {
    // Apply theme
    ...
    }, ['overview', 'users', 'config', 'security'].map(tab => React.createElement('button', {
        key: tab,
    ...
    }, 'Authentication Status'), isAuthenticated ? React.createElement('div', {}, React.createElement('div', {
        style: {
            display: 'flex',
            alignItems: 'center',
    ...
    }, currentSession?.user.name?.charAt(0) || currentSession?.user.email.charAt(0)), React.createElement('div', {}, React.createElement('div', {
        style: {
            fontWeight: 600,
            color: appliedTheme.colors.textPrimary
        }
    }, currentSession?.user.name || currentSession?.user.email), React.createElement('div', {
        style: {
            fontSize: appliedTheme.font.sizeSm,
            color: appliedTheme.colors.textSecondary
        }
    }, `Role: ${currentSession?.user.role || 'user'}`))), React.createElement('button', {
        onClick: onSignOut,
        style: {
    ...
    }, 'Sign Out')) : React.createElement('div', {}, React.createElement('p', {
        style: {
            marginBottom: appliedTheme.spacing.md,
            color: appliedTheme.colors.textSecondary
        }
    }, 'Not authenticated'), React.createElement('div', {
        style: {
            display: 'flex',
    ...
    const renderUsers = () => (React.createElement('div', {}, React.createElement('div', {
        style: {
            display: 'flex',
    ...
    }, 'User Management'), (userRole === 'admin') && React.createElement('button', {
        onClick: () => setShowSignUp(true),
        style: {
            padding: `${appliedTheme.spacing.sm} ${appliedTheme.spacing.md}`,
            backgroundColor: appliedTheme.colors.secondary,
        ...
        }, user.name?.charAt(0) || user.email.charAt(0)), React.createElement('div', {}, React.createElement('h4', {
            style: {
        ...
        }, user.name || user.email), React.createElement('p', {
            style: {
                fontSize: appliedTheme.font.sizeSm,
                color: appliedTheme.colors.textSecondary,
                margin: 0
        ...
        }, user.isVerified ? '✓ Verified' : '⚠ Unverified'), locked && React.createElement('span', {
            style: { color: appliedTheme.colors.danger }
        }, '🔒 Locked'), attempts > 0 && React.createElement('span', {
            style: { color: appliedTheme.colors.warning }
        }, `${attempts} failed attempts`))), (userRole === 'admin') && React.createElement('div', {
            style: {
                display: 'flex',
        ...
        }, 'Verify'), locked && React.createElement('button', {
        ...
        }, 'Unlock'), React.createElement('button', {
    ...
    const renderConfig = () => (React.createElement('div', {}, React.createElement('div', {
        style: {
            display: 'flex',
            justifyContent: 'space-between',
            alignItems: 'center',
    ...
    }, 'Authentication Configuration'), (userRole === 'admin') && React.createElement('button', {
    ...
    }, editingConfig ? 'Cancel' : 'Edit Config')), authConfig ? React.createElement('div', {
        style: {
            backgroundColor: appliedTheme.colors.surface,
            borderRadius: appliedTheme.borders.borderRadius,
            padding: appliedTheme.spacing.lg,
    ...
    const renderSecurity = () => (React.createElement('div', {}, React.createElement('h2', {
        style: {
            fontSize: appliedTheme.font.sizeXl,
            fontWeight: 600,
            marginBottom: appliedTheme.spacing.lg,
    ...
    }, 'Security Overview'), React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.lg
        }
    ...
    React.createElement('div', {
    ...
    }, 'Failed Login Attempts'), Object.keys(loginAttempts).length > 0 ? React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.sm
        }
        ...
        }, user?.email || userId), React.createElement('span', {
            style: {
    ...
    React.createElement('div', {
    ...
    }, 'Locked Users'), Object.keys(lockedUsers).length > 0 ? React.createElement('div', {
        style: {
            display: 'grid',
            gap: appliedTheme.spacing.sm
        }
        ...
        }, user?.email || userId), React.createElement('div', {
            style: {
                fontSize: appliedTheme.font.sizeSm,
                color: appliedTheme.colors.textSecondary
            }
        }, `Unlocks at: ${formatDate(unlockTime)}`)), (userRole === 'admin') && React.createElement('button', {
    ...
    // Tab content
    !loading && activeTab === 'overview' && renderOverview(), !loading && activeTab === 'users' && renderUsers(), !loading && activeTab === 'config' && renderConfig(), !loading && activeTab === 'security' && renderSecurity(), 
    // Sign In Modal
